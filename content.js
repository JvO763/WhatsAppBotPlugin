...
async function handleMessage() {
    ...
    let allMessages = document.querySelectorAll("div.message-in, div.message-out");
    ...
    for (let i = allMessages.length - 1; i >= 0; i--) {
        ...
        let textSpan = msg.querySelector("span[data-testid='selectable-text']");
        if (!textSpan) continue;

        let text = textSpan.innerText.trim();
        if (!text) continue;

        if (!lastUserMessage && msg.classList.contains("message-in")) {
            lastUserMessage = text;
        }

        if (!lastBotMessage && msg.classList.contains("message-out")) {
            lastBotMessage = text;
        }

        if (lastUserMessage && lastBotMessage) break;
    }
